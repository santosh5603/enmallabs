  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 w-14 h-14 bg-[#0075de] text-white rounded-full shadow-[0_4px_20px_rgba(0,117,222,0.3)] flex items-center justify-center transition-all z-40 hover:bg-[#005bb5] hover:scale-110 active:scale-95 ${isOpen ? 'scale-0 opacity-0 pointer-events-none' : 'scale-100 opacity-100'}`}
          ...
          >
            ...
            <div className="h-16 bg-[#f6f5f4] border-b border-[#e6e6e6] flex items-center justify-between px-4 shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-[#0075de]/15 flex items-center justify-center">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-white">
              {messages.map((msg, i) => (
                <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-1 ${msg.role === 'user' ? 'bg-[#0075de]/20' : 'bg-[#0075de]'}`}>
                    {msg.role === 'user' ? <User className="w-3 h-3 text-[#0075de]" /> : <Bot className="w-3 h-3 text-white" />}
                  </div>
                  <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${msg.role === 'user' ? 'bg-[#0075de] text-white rounded-tr-sm' : 'bg-[#f6f5f4] border border-[#e6e6e6] text-black rounded-tl-sm'}`}>
                    {msg.role === 'user' ? (
              ...
              {isTyping && (
                <div className="flex gap-3">
                  <div className="w-6 h-6 rounded-full bg-[#0075de] flex items-center justify-center shrink-0 mt-1">
            ...
            <div className="p-3 bg-[#f6f5f4] border-t border-[#e6e6e6] shrink-0">
              <form onSubmit={handleSend} className="relative flex items-center">
                <input
                  ...
                  className="w-full bg-white border border-[#e6e6e6] rounded-xl text-black pl-4 pr-10 py-3 focus:outline-none focus:border-[#0075de]/50 text-sm transition-colors placeholder:text-black/30"
                />
                <button
                  type="submit"
                  disabled={!input.trim() || isTyping}
                  className="absolute right-2 p-1.5 rounded-lg bg-[#0075de] text-white hover:bg-[#005bb5] disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
